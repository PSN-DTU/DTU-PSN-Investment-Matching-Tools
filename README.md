...
6) Scroll through the cells and update the file paths where necessary; for example the path to requirements should read /content/requirements.txt and the paths to the company blacklist and investment portfolios also need to be defined. To quickly define the paths, right click on the desired file and select "Copy path" then paste it into the relevant location in the code.

   <img width="435" height="289" alt="Screenshot 2026-07-31 154608" src="https://github.com/user-attachments/assets/0e402359-034b-4cca-8063-72d3eccb4911" />

7) Run Cell 1 to install necessary packages
8) Run Cell 2 to update directory
9) IF ONE WISHES TO CREATE A NEW COMPANY BLACKLIST: Run Cells 3 and 4, otherwise skip this step
   a) Requires an openai_api_key
   b) The LLM will attempt to find parent companies and subsidiaries of the companies supplied
10) Run Cell 5 to match investment data to the blacklisted company list
...
11) Add the matched company along with the amount and asset manager to the tab related to the relevant month in the Investment Template worksheet. The Summary page will autofill as data is entered. It is important that a consistent company name is used in the "Company" column across the different asset managers so that the sheet can accurately tabulate investment amounts. For example: the investments VOLKSWAGEN INTL FIN NV PERP, VW 2 1/4 10/01/27, Volkswagen Pref should all be listed under a common company name like Volkswagen.
12) If using different asset managers than the ones listed, go to the 'January' tab and change cells N8, N9, N10, etc. to the names of the asset managers. Then edit the formula for cells M8, M9, M10, etc so that the text in the formula between the two asterisks is the same as the text entered into N8, N9, N10, etc. Copy this block of edited cells and paste them in the same location for all the other monthly tabs. If using more than 3 asset managers, format the Summary tab accordingly by cutting and pasting the blacklist summary columns the required amount of columns away and adjusting the formulas for each subsequent asset manager. 
